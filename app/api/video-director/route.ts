...
export async function POST(request: NextRequest) {
  ...
  try {
    const { audio, narration } = await request.json();
...
【タスク】
音声を聞いて、完成動画から削除すべき区間を特定してください：
- 言い淀み（「えー」「あのー」等）
- 噛み・言い直し（同じ文を2回言っている場合は失敗した方）
- 台本にない雑談・咳・無関係な音
- 長すぎる間

確信がある箇所のみ挙げてください。全体が問題なければ空配列でOK。

以下のJSON形式のみで回答：
{"remove": [{"start": 秒数, "end": 秒数, "reason": "短い理由"}], "advice": "全体への一言アドバイス"}`;
    ...
    return NextResponse.json({ remove, advice: parsed.advice ?? "" });
  } catch (error) {
